            ...
            <div class="p-4 mt-16 min-h-screen">
                <div class="grid grid-cols-1">
                    <div class="card w-auto bg-white-custom shadow-sm">
                        <div class="card-body">
                            <h3 class="lg:text-xl font-medium text-black-custom text-base mb-4">Edit Mapel
                            </h3>
                            <form action="{{ route('master_mapel.update', $mapel->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text"
                                        ...
                                        name="nama_mapel" value="{{ $mapel->nama_mapel }}" id="nama-jurusan"
                                        placeholder="Nama jurusan" required>
                                </div>
                                <div class="mb-4">
                                    <label for="jurusan"
                                        class="block mb-3 text-sm font-medium text-black-custom">Guru</label>
                                    @if ($errors->has('id_guru'))
                                        <div class="text-red-500 text-sm mb-2">{{ $errors->first('id_guru') }}</div>
                                    @endif
                                    <select name="id_guru" id="jurusan"
                                        class="border border-gray-custom text-sm rounded-lg block py-3 px-2 text-black-custom w-full md:w-md focus:outline-gray-custom font-medium"
                                        required>
                                        <option value="">-- Pilih Guru --</option>
                                        @foreach ($guru as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('id_guru', $item->id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->nama }}
                                            </option>
                                        @endforeach
                                    </select>
